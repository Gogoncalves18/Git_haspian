...
while True: #Looping para quando a variavel SAIR for S
    ...
    while True:  #Validacao da idade com controle de excesao 
        try:
            ...
            break
    inf_p['sexo']='a' #Setagem do sexo apenas para fazer o controle de entrada do valor
    while inf_p['sexo'] not in 'MF': #Validaao da resposta apenas em M ou F
    ...
    while sair not in 'SN': #Validacao da resposta de saida do programa
        sair=str(input(f'Finalizar os dados? [S] ou [N]')).upper()[0] #Sair do programa
    if sair in 'S':
        break
print('-:'*30)
...
idade_med=num_m=0 #Variaveis de controle estatistico para media de idade e numero de mulheres
...
for v in lst_ps:
    if v["idade"] > idade_med/len(lst_ps):
        p_up_med.append(v["nome"]) #Separacao das pessoas acima da media de idade do grupo
print(f'A media de idade do grupo é de {idade_med/len(lst_ps)} anos.')
print(f'Temos {num_m} mulher(es) no grupo.')
print(f'Temos as seguintes pessoas {p_up_med} que estao acima da media de idade de {idade_med/len(lst_ps)} anos.')
